...
client.on("message", msg => {
    ...
    const embedlul = new Discord.RichEmbed()
      .setColor(0x00AE86)
      .setDescription( msg.author + " Reklam Yasak AQ")
...
const youtube = new YouTube('AIzaSyDRXCjPlUDVOmwZ-SCvaVJ7TqruGAHyjZk');
const queue = new Map();
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'sa') {
		if (!msg.guild.member(msg.author).hasPermission("BAN_MEMBERS")) {
			msg.author.sendMessage('**Cami de değilsin oç**'); 
		} else {
		msg.reply('Camide değilsin oç^^');
		}
	} 
});
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'Bum be') {
		if (!msg.guild.member(msg.author).hasPermission("BAN_MEMBERS")) {
			msg.author.sendMessage('YARRAK'); 
		} else {
		msg.reply('YARRAK');
		}
	}
...
client.on("message", msg => {
  const uyarıembed = new Discord.RichEmbed()
    .setColor(0x00AE86)
    .setDescription(":crown: " + msg.author + "rEkLaMI kEs  :angry: :rage: ")
